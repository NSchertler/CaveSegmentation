...
double CaveSizeCalculatorVoronoi::CalculateDistance(const RegularUniformSphereSampling & sphereSampling, const std::vector<std::vector<double>>& sphereDistances, const std::vector<std::vector<Vector>>& distanceGradient, const std::vector<PositionValue>& sphereDistanceMaxima, const std::vector<PositionValue>& sphereDistanceMinima, SphereVisualizer& visualizer, int iVert)
{
	...
	for (auto it = sphereSampling.begin(); it != sphereSampling.end(); ++it)
	{
		...
		if (isOnVoronoiEdge)
		{
			...
			int left = (int)(theta * imWidth / (2 * M_PI));
			int top = (int)(phi * imHeight / M_PI);
			int right = (int)((theta + wTheta) * imWidth / (2 * M_PI));
			int bottom = (int)((phi + wPhi) * imHeight / M_PI);
			visualizer.DrawRect(left, top, right - left, bottom - top, SphereVisualizer::VORONOI_COLOR);
#endif
...
double CaveSizeCalculatorLineFlow::CalculateDistance(const RegularUniformSphereSampling & sphereSampling, const std::vector<std::vector<double>>& sphereDistances, const std::vector<std::vector<Vector>>& distanceGradient, const std::vector<PositionValue>& sphereDistanceMaxima, const std::vector<PositionValue>& sphereDistanceMinima, SphereVisualizer& visualizer, int iVert)
{
	...
	LineFlow<true>(separatingCircle, sphereSampling, sphereDistances, distanceGradient, +1.0, partialAverageSize, partialLineLength, visualizer
#if defined(DRAW_DEBUG_IMAGES) || defined(WRITE_SPHERE_STATS)
		, tempDirectory + L"/separatingCircle_" + std::to_wstring(iVert)
#endif
		);
	...
	for (auto& sample : separatingLine)
	{
		...
		visualizer.FillCircle((int)(imWidth * theta / 2 / M_PI), (int)(imHeight * phi / M_PI), 2, SphereVisualizer::SEPARATING_LINE_COLOR);
	}
#endif
	...
	FindOptimalMeanAverage(separatingLineLocalMinima, CIRCLE_SUBSAMPLING_MIN_DISTANCE, CIRCLE_SUBSAMPLING_MAX_DISTANCE, meanAverageWorkspace, representativeMinimaOnSeparatingLine);
	if (representativeMinimaOnSeparatingLine.size() == 0)
	{
		usedLine = &separatingLine;
		FindOptimalMeanAverage(separatingLine, CIRCLE_SUBSAMPLING_MIN_DISTANCE, CIRCLE_SUBSAMPLING_MAX_DISTANCE, meanAverageWorkspace, representativeMinimaOnSeparatingLine);
	}
	...
	for (int minimumIndex : representativeMinimaOnSeparatingLine)
	{
		...
		visualizer.FillCircle((int)(imWidth * theta / 2 / M_PI), (int)(imHeight * phi / M_PI), 4, SphereVisualizer::SEPARATING_LINE_COLOR);
#endif
		...
		for (double beta = betaMin; beta <= betaMax; beta += LINE_POINT_DISTANCE)
		{
			...
			visualizer.FillCircle((int)theta, (int)phi, 2, SphereVisualizer::SEPARATING_CIRCLE_COLOR);
#endif
		}

		//Let the line flow towards the valleys
		LineFlow<false>(minimaLine, sphereSampling, sphereDistances, distanceGradient, -1.0, partialAverageSize, partialLineLength, visualizer
#if defined(DRAW_DEBUG_IMAGES) || defined(WRITE_SPHERE_STATS)
			, tempDirectory + L"/minimaLineFlow_" + std::to_wstring(iVert) + L"_" + std::to_wstring(minimumIt)
#endif
			);
		...
		for (auto& p : minimaLine)
		{
			...
			visualizer.FillCircle((int)theta, (int)phi, 2, SphereVisualizer::FLOW_COLOR);
		}
#endif
