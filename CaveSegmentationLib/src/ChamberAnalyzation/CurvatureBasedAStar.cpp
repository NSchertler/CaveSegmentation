...
void CurvatureBasedAStar::FindChambers(const ICaveData& data, std::vector<int>& segmentation, bool verbose)
{
	...
	//Find local maxima
	EdgeCurvatureAccessor curvatureAccessor(data);
	for (int i = 0; i < data.Skeleton()->edges.size(); ++i)
	{
		auto& edge = data.Skeleton()->edges.at(i);

		double edgeValue = data.CaveSizeCurvature(i);

		//check incoming and outgoing neighbors
		bool isMaximum = IsEdgeLocalMaximum(data, curvatureAccessor, edge.first, edge.second, edgeValue);
		if (isMaximum)
			isMaximum = IsEdgeLocalMaximum(data, curvatureAccessor, edge.second, edge.first, edgeValue);
...
#pragma omp parallel for
	for (int i = 0; i < (int)data.Skeleton()->vertices.size(); ++i)
		ufCaveSizeMaxima[i] = data.CaveSize(i);
	...
	for (int i : preservedEdges)
	{
		...
		double derivative = data.CaveSizeDerivative(i);
		if (derivative < 0)
		{
		...
		double entranceProb = entranceProbability(data.CaveSizeCurvature(i));
		double size = (data.CaveSize(edge.first) + data.CaveSize(edge.second)) / 2;
