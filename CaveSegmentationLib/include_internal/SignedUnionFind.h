...
public:
	...
	//Iterators	
	class RootIterator : public std::iterator<std::forward_iterator_tag, index_t>
	{
	public:
		RootIterator(const SignedUnionFindBase<UseSign>& uf)
			: uf(uf), currentItem(-1)
		{
			operator++();
		}

		RootIterator(const SignedUnionFindBase<UseSign>& uf, size_t current)
			: uf(uf), currentItem(current)
		{}
		...
		{
			++currentItem;
			while (currentItem < uf.size() && uf.parentIndices.at(currentItem) != currentItem)
				++currentItem;
			return *this;
		}

		bool operator!=(const RootIterator& other)
		{
			return currentItem != other.currentItem;
		}

	private:
		const SignedUnionFindBase<UseSign>& uf;
		size_t currentItem;
	};
	...
	{
	public:
		RootHelper(const SignedUnionFindBase<UseSign>& uf) : uf(uf) {}
		RootIterator begin() { return RootIterator(uf); }
		RootIterator end() { return RootIterator(uf, uf.size()); }
...
public:
	...
	void addItems(int count, bool negative)
	{
		parentIndices.resize(parentIndices.size() + count);
		for (unsigned int i = parentIndices.size() - count; i < parentIndices.size(); ++i)
			parentIndices.at(i) = (index_t)i;
		ranks.resize(ranks.size() + count, 0);
		if (UseSign)
			signs.resize(signs.size() + count, negative);
	}
	...
	RootHelper roots() { return RootHelper(*this); }	
