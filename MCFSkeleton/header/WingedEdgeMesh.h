...
namespace WingedEdge
{
	...
	public: //------------------------------------------------------ iterator types
		...
		public:
			...
			/// Default constructor
			Vertex_iterator(Vertex v = Vertex(), const WingedEdgeMesh* m = NULL) : hnd_(v), mesh_(m)
			{
		...
		public:
			...
			/// Default constructor
			Edge_iterator(Edge e = Edge(), const WingedEdgeMesh* m = NULL) : hnd_(e), mesh_(m)
			{
				if (mesh_ && mesh_->garbage()) while (mesh_->is_valid(hnd_) && mesh_->is_deleted(hnd_)) ++hnd_.idx_;
			}
		...
		public:
			...
			/// Default constructor
			Face_iterator(Face f = Face(), const WingedEdgeMesh* m = NULL) : hnd_(f), mesh_(m)
			{
				if (mesh_ && mesh_->garbage()) while (mesh_->is_valid(hnd_) && mesh_->is_deleted(hnd_)) ++hnd_.idx_;
			}
	...
	public: //---------------------------------------------------- circulator types
		...
		public:
			...
			/// default constructor
			Edge_around_vertex(const WingedEdgeMesh* m = NULL, Vertex v = Vertex()) : mesh_(m)
			{
	...
	public: //-------------------------------------------- constructor / destructor
		...
		//WingedEdgeMesh& operator=(const WingedEdgeMesh& rhs);

		/// assign \c rhs to \c *this. does not copy custom properties.
		//WingedEdgeMesh& assign(const WingedEdgeMesh& rhs);

		/// assign \c rhs to \c *this. performs a shallow copy of all properties.
		//void shallow_copy(const WingedEdgeMesh& rhs);
	...
	public: //--------------------------------------------------- helper functions
		...
		void remove_edge(Edge e)
		{
			...
			for (unsigned int i = 0; i < v2_adj.size(); i++)
			{
				// We will kill 'ei'
				Edge ei = v2_adj[i];

				for (unsigned int j = i + 1; j < v2_adj.size(); j++)
				{
					Edge ej = v2_adj[j];
					...
					if (same_edge(ei, ej))
					{
						...
						remove_edge(econn_[ei].vertex0_, ei);
						remove_edge(econn_[ei].vertex1_, ei);
	...
	protected: //------------------------------------------------------- private data
		...
		ElementProperty<Edge_connectivity, Edge>          econn_;
		...
		ElementProperty<Vector, Vertex>   vpoint_;
