...
public: //---------------------------------------------- low-level connectivity
	...
	bool has_faces(Edge e)
	{
		...
		return
			!fdeleted_[hconn_[halfedge(e, 0)].face_] ||
			!fdeleted_[hconn_[halfedge(e, 1)].face_];		
	}
...
public: //--------------------------------------------------- property handling
    ...
    template <class T> ElementProperty<T, Vertex> add_vertex_property(const std::string& name, const T t=T())
    {
        return ElementProperty<T, Vertex>(vprops_.add<T>(name, t));
    }
    /** add a halfedge property of type \c T with name \c name and default value \c t.
     fails if a property named \c name exists already, since the name has to be unique.
     in this case it returns an invalid property */
    template <class T> ElementProperty<T, Halfedge> add_halfedge_property(const std::string& name, const T t=T())
    {
        return ElementProperty<T, Halfedge>(hprops_.add<T>(name, t));
    }
    /** add a edge property of type \c T with name \c name and default value \c t.
     fails if a property named \c name exists already, since the name has to be unique.
     in this case it returns an invalid property */
    template <class T> ElementProperty<T, Edge> add_edge_property(const std::string& name, const T t=T())
    {
        return ElementProperty<T, Edge>(eprops_.add<T>(name, t));
    }
    /** add a face property of type \c T with name \c name and default value \c t.
     fails if a property named \c name exists already, since the name has to be unique.
     in this case it returns an invalid property */
    template <class T> ElementProperty<T, Face> add_face_property(const std::string& name, const T t=T())
    {
        return ElementProperty<T, Face>(fprops_.add<T>(name, t));
    }
    ...
    template <class T> ElementProperty<T, Vertex> get_vertex_property(const std::string& name) const
    {
        return ElementProperty<T, Vertex>(vprops_.get<T>(name));
    }
    /** get the halfedge property named \c name of type \c T. returns an invalid
     Vertex_property if the property does not exist or if the type does not match. */
    template <class T> ElementProperty<T, Halfedge> get_halfedge_property(const std::string& name) const
    {
        return ElementProperty<T, Halfedge>(hprops_.get<T>(name));
    }
    /** get the edge property named \c name of type \c T. returns an invalid
     Vertex_property if the property does not exist or if the type does not match. */
    template <class T> ElementProperty<T, Edge> get_edge_property(const std::string& name) const
    {
        return ElementProperty<T, Edge>(eprops_.get<T>(name));
    }
    /** get the face property named \c name of type \c T. returns an invalid
     Vertex_property if the property does not exist or if the type does not match. */
    template <class T> ElementProperty<T, Face> get_face_property(const std::string& name) const
    {
        return ElementProperty<T, Face>(fprops_.get<T>(name));
    }
    ...
    template <class T> ElementProperty<T, Vertex> vertex_property(const std::string& name, const T t=T())
    {
        return ElementProperty<T, Vertex>(vprops_.get_or_add<T>(name, t));
    }
    /** if a halfedge property of type \c T with name \c name exists, it is returned.
     otherwise this property is added (with default value \c t) */
    template <class T> ElementProperty<T, Halfedge> halfedge_property(const std::string& name, const T t=T())
    {
        return ElementProperty<T, Halfedge>(hprops_.get_or_add<T>(name, t));
    }
    ...
    /** if an edge property of type \c T with name \c name exists, it is returned.
     otherwise this property is added (with default value \c t) */
    template <class T> ElementProperty<T, Edge> edge_property(const std::string& name, const T t=T())
    {
        return ElementProperty<T, Edge>(eprops_.get_or_add<T>(name, t));
    }
    /** if a face property of type \c T with name \c name exists, it is returned.
     otherwise this property is added (with default value \c t) */
    template <class T> ElementProperty<T, Face> face_property(const std::string& name, const T t=T())
    {
        return ElementProperty<T, Face>(fprops_.get_or_add<T>(name, t));
    }


    /// remove the vertex property \c p
    template <class T> void remove_vertex_property(ElementProperty<T, Vertex>& p)
    {
        vprops_.remove(p);
    }
    /// remove the halfedge property \c p
    template <class T> void remove_halfedge_property(ElementProperty<T, Halfedge>& p)
    {
        hprops_.remove(p);
    }
    /// remove the edge property \c p
    template <class T> void remove_edge_property(ElementProperty<T, Edge>& p)
    {
        eprops_.remove(p);
    }
    /// remove the face property \c p
    template <class T> void remove_face_property(ElementProperty<T, Face>& p)
    {
        fprops_.remove(p);
    }
...
protected: //------------------------------------------------------- private data
    ...
    ElementProperty<Vertex_connectivity, Vertex>      vconn_;
	ElementProperty<Halfedge_connectivity, Halfedge>  hconn_;
	ElementProperty<Face_connectivity, Face>          fconn_;

	ElementProperty<bool, Vertex>  vdeleted_;
	ElementProperty<bool, Edge>    edeleted_;
	ElementProperty<bool, Face>    fdeleted_;

	ElementProperty<Point, Vertex>   vpoint_;
	ElementProperty<Normal, Vertex>  vnormal_;
	ElementProperty<Normal, Face>    fnormal_;
