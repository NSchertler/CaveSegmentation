...
template <class T, class TParent> class ElementProperty : public Property<T>
{
public:

	/// default constructor
	explicit ElementProperty() {}
	explicit ElementProperty(Property<T> p) : Property<T>(p) {}

	/// access the data stored for vertex \c v
	typename Property<T>::reference operator[](TParent p)
	{
		return Property<T>::operator[](p.idx());
	}

	/// access the data stored for vertex \c v
	typename Property<T>::const_reference operator[](TParent p) const
	{
		return Property<T>::operator[](p.idx());
	}
};
