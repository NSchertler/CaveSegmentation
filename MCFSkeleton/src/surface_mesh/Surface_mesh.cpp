...
garbage_collection()
{
    ...
    ElementProperty<Vertex, Vertex>      vmap = add_vertex_property<Vertex>("v:garbage-collection");
	ElementProperty<Halfedge, Halfedge>  hmap = add_halfedge_property<Halfedge>("h:garbage-collection");
	ElementProperty<Face, Face>          fmap = add_face_property<Face>("f:garbage-collection");
    for (i=0; i<nV; ++i)
