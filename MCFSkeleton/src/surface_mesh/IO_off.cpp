...
bool read_off(Surface_mesh& mesh, const std::string& filename)
{
    ...
    // properties
    ElementProperty<Surface_mesh::Normal, Surface_mesh::Vertex>  normals;
    ElementProperty<Surface_mesh::Texture_coordinate, Surface_mesh::Vertex>  texcoords;
...
bool write_off(const Surface_mesh& mesh, const std::string& filename)
{
	...
	std::unordered_map<int, unsigned int> ids;

    // vertices
	int i = 0;
    ElementProperty<Surface_mesh::Point, Surface_mesh::Vertex> points = mesh.get_vertex_property<Point>("v:point");
    for (Surface_mesh::Vertex_iterator vit=mesh.vertices_begin(); vit!=mesh.vertices_end(); ++vit)
    {
		ids[((Surface_mesh::Vertex)vit).idx()] = i++;
    ...
    for (Surface_mesh::Face_iterator fit=mesh.faces_begin(); fit!=mesh.faces_end(); ++fit)
    {
        ...
        do
        {
            fprintf(out, " %d", ids.at(((Surface_mesh::Vertex)fvit).idx()));
        }
        while (++fvit != fvend);
        fprintf(out, "\n");
    }
