...
.DESCRIPTION
Die Backup-Subscription-Funktion speichert das aktuelle Microsoft Azure-Abonnement (Get-AzureSubscription -Current) und das zugehörige Speicherkonto sowie das von diesem Skript geänderte Abonnement ($UserSpecifiedSubscription) und das zugehörige Speicherkonto im Skriptbereich. Das Speichern der Werte bietet Ihnen die Möglichkeit, das ursprüngliche aktuelle Abonnement und Speicherkonto mithilfe einer Funktion wie Restore-Subscription mit dem aktuellen Status wiederherzustellen, falls sich der aktuelle Status geändert hat.
...
.DESCRIPTION
Die Get-AzureSQLDatabaseConnectionString-Funktion erstellt eine Verbindungszeichenfolge für die Verbindungsherstellung mit einer Microsoft Azure SQL-Datenbank.

.PARAMETER  DatabaseServerName
Dient zum Angeben eines vorhandenen Datenbankservers im Microsoft Azure-Abonnement. Alle Microsoft Azure SQL-Datenbanken müssen einem SQL-Datenbankserver zugeordnet werden. Verwenden Sie zum Abrufen des Servernamens das Get-AzureSqlDatabaseServer-Cmdlet (Azure-Modul). Dieser Parameter muss angegeben werden.
